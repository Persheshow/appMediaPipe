...
export default function App() {
  ...
  // Nuovo stato: Lato di Registrazione
  const [cameraSide, setCameraSide] = useState('LEFT');

  const { videoRef, canvasRef, isLoading, error, validReps, noReps, faults, angles, reset } = usePose(exercise, isActive, cameraSide);
      ...
      {!isActive ? (
        <div className="w-full max-w-md flex flex-col gap-8 mt-6">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 py-4 rounded-xl text-sm font-semibold uppercase tracking-wider transition-colors
                  ${cameraSide === 'LEFT' ? 'bg-indigo-600 text-white border-2 border-indigo-400' : 'bg-slate-800 text-slate-400 border-2 border-slate-700'}`}
              >
                Sinistra
              </button>
              <button
                ...
                className={`flex-1 py-4 rounded-xl text-sm font-semibold uppercase tracking-wider transition-colors
                  ${cameraSide === 'RIGHT' ? 'bg-indigo-600 text-white border-2 border-indigo-400' : 'bg-slate-800 text-slate-400 border-2 border-slate-700'}`}
              >
                Destra
              </button>
            </div>
          ...
          <button
            ...
            className="mt-4 w-full py-4 bg-emerald-600 hover:bg-emerald-500 active:bg-emerald-700 rounded-xl font-bold text-lg transition-colors text-white"
          >
            Avvia Analisi
          </button>
        </div>
          ...
          <section className="w-full bg-slate-900 border border-slate-800 rounded-xl p-4 flex justify-around mb-4 shadow-lg">
            ...
            <div className="text-center">
              ...
              <p className="text-sm font-mono text-slate-300 mt-1">
                ...
                {exercise === 'DEADLIFT' && (`H: ${angles.primary ? Math.round(angles.primary) : '--'}° K: ${angles.secondary ? Math.round(angles.secondary) : '--'}°`)}
                {exercise === 'OVERHEAD_PRESS' && (`E: ${angles.primary ? Math.round(angles.primary) : '--'}° T: ${angles.secondary ? Math.round(angles.secondary) : '--'}°`)}
              </p>
            </div>
          </section>
